...
class BytecodeClass:
    def __init__(self):
        ...
        # self.func_arg_counts: dict[str, tuple[int, int]] = {}
        self.tk_names = []
                ...
                if func_name == "var2bytes" or func_name == "bytes2var":
                    ...
                    for builtin_func_arg_element in builtin_func_arg_elements:
                        if builtin_func_arg_element[0] == func_name:
                            last_el = builtin_func_arg_element[1][1]
                            if last_el == INT_MAX:
                                last_el = "INT_MAX"
                            f.write(
                                '\t{ "'
...
def write_bytecode_json(dir: Path, bytecode_classes: list[BytecodeClass]) -> None:
    ...
    for bytecode_class in bytecode_classes:
        # if len(bytecode_class.func_arg_counts) == 0:
        #     for func_name in bytecode_class.func_names:
        #         for builtin_func_arg_element in builtin_func_arg_elements:
        #             if builtin_func_arg_element[0] == func_name:
        #                 counts = builtin_func_arg_element[1]
        #                 if func_name == "var2bytes" or func_name == "bytes2var":
        #                     if bytecode_class.bytecode_rev_num in greater_than_3_1_versions:
        #                         counts = (1, 2)
        #                 bytecode_class.func_arg_counts[func_name] = counts
        #                 break
        #         if func_name not in bytecode_class.func_arg_counts:
        #             raise ValueError(f"Function {func_name} not found in builtin_func_arg_elements")
        bytecode_json.append(
            {
                ...
                "func_names": bytecode_class.func_names,
                # "func_arg_counts": bytecode_class.func_arg_counts,
                "tk_names": bytecode_class.tk_names,
            }
        )
    with open(dir / "bytecode_versions.json", "w") as f:
...
# write_bytecode_json(dir=our_dir / "misc", bytecode_classes)
