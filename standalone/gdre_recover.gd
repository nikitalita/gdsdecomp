...
func _ready():
	...
	# TODO: remove me
	load_test()
	$VersionText.text = GDRESettings.get_version_string()
	var file_list: Tree = $FileTree
...
func clear():
	if (userroot != null):
		...
		userroot = null
	$FileTree.clear()
	root = $FileTree.create_item()
	root.set_cell_mode(0, TreeItem.CELL_MODE_CHECK)
	root.set_checked(0, true)
	root.set_editable(0, true)
	root.set_icon(0, get_window().get_theme_icon("folder", "FileDialog"))
	root.set_text(0, "res://")
...
func add_file_to_item(p_item: TreeItem, p_fullname: String, p_name: String, p_size: int, p_icon: Texture2D,  p_error: String, p_enc: bool):
	...
	if (pp == -1):
		# Add file
		var item: TreeItem = $FileTree.create_item(p_item);
...
#
		...
		item.set_checked(0, true);
		item.set_editable(0, true);
		item.set_icon(0, get_window().get_theme_icon("folder", "FileDialog"));
...
func extract_and_recover(output_dir: String):
	...
	if (err):
		popup_error_box("Error: Could not extract files!!", "Error", get_window())
		return
	# check if ExtractOnly is pressed
	if ($RadioButtons/ExtractOnly.is_pressed()):
		report += "Total files extracted: " + String.num(files_to_extract.size()) + "\n"
		popup_error_box(report, "Info", get_window())
