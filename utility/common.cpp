...
namespace {
Error _wget_sync(const String &p_url, Ref<FileAccess> response, int retries, const Vector<String> &extra_headers, float *p_progress, bool *p_cancelled) {
#define WGET_CANCELLED_CHECK()         \
	...
	auto _retry = [&](Error retry_err) {
		...
		return _wget_sync(p_url, response, retries, extra_headers, p_progress, p_cancelled);
	};
	size_t downloaded = 0;
...
Error gdre::wget_sync(const String &p_url, Vector<uint8_t> &response, int retries, const Vector<String> &extra_headers, float *p_progress, bool *p_cancelled) {
	...
	Error err = _wget_sync(p_url, fa, retries, extra_headers, p_progress, p_cancelled);
	if (err) {
		return err;
	}
