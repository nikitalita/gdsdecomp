...
Error ResourceLoaderCompat::open(FileAccess *p_f, bool p_no_resources, bool p_keep_uuid_paths) {
	error = OK;
...
RES ResourceLoaderCompat::get_external_resource(const int subindex) {
...
String ResourceLoaderCompat::get_external_resource_path(const RES &res) {
...
RES ResourceLoaderCompat::get_external_resource(const String &path) {
	for (int i = 0; i < external_resources.size(); i++) {
...
RES ResourceLoaderCompat::get_external_resource_by_id(const String &id) {
	for (int i = 0; i < external_resources.size(); i++) {
...
bool ResourceLoaderCompat::has_external_resource(const RES &res) {
	for (int i = 0; i < external_resources.size(); i++) {
		if (external_resources[i].cache == res) {
			return true;
		}
...
RES ResourceLoaderCompat::instance_internal_resource(const String &path, const String &type, const String &id) {
	RES res;
...
RES ResourceLoaderCompat::get_internal_resource(const int subindex) {
...
RES ResourceLoaderCompat::get_internal_resource_by_id(const String &id) {
...
RES ResourceLoaderCompat::get_internal_resource(const String &path) {
...
bool ResourceLoaderCompat::has_internal_resource(const RES &res) {
...
String ResourceLoaderCompat::get_resource_path(const RES &res) {
	if (res.is_null()) {
		return "";
	}
...
String ResourceLoaderCompat::get_ustring(FileAccess *f) {
	int len = f->get_32();
	Vector<char> str_buf;
	if (len == 0) {
		return String();
...
RES ResourceLoaderCompat::make_dummy(const String &path, const String &type, const String &id) {
...
RES ResourceLoaderCompat::set_dummy_ext(const uint32_t erindex) {
...
RES ResourceLoaderCompat::set_dummy_ext(const String &path, const String &exttype) {
	for (int i = 0; i < external_resources.size(); i++) {
		if (external_resources[i].path == path) {
...
void ResourceLoaderCompat::advance_padding(FileAccess *f, uint32_t p_len) {
...
String ResourceLoaderCompat::_write_rlc_resources(void *ud, const RES &p_resource) {
	ResourceLoaderCompat *rsi = (ResourceLoaderCompat *)ud;
	return rsi->_write_rlc_resource(p_resource);
}

String ResourceLoaderCompat::_write_rlc_resource(const RES &res) {
	String path = get_resource_path(res);
...
Error ResourceLoaderCompat::parse_variant(Variant &r_v) {
	...
	// print_bl("find property of res_type: %d", res_type);
	...
	switch (type) {
		...
		case VariantBin::VARIANT_IMAGE: {
			...
			if (ImageParserV2::decode_image_v2(f, r_v, hacks_for_deprecated_v2img_formats, convert_v2image_indexed) != OK) {
...
void ResourceLoaderCompat::save_ustring(FileAccess *f, const String &p_string) {
...
Error ResourceLoaderCompat::write_variant_bin(FileAccess *fa, const Variant &p_property, const PropertyInfo &p_hint) {
...
void ResourceLoaderCompat::get_dependencies(FileAccess *p_f, List<String> *p_dependencies, bool p_add_types, bool only_paths) {
	open(p_f);
	if (error) {
		return;
	}
...
Error ResourceLoaderCompat::open_text(FileAccess *p_f, bool p_skip_first_tag) {
	error = OK;
...
Error ResourceLoaderCompat::fake_load_text(FileAccess *p_f, const String &p_path) {
	if (error) {
		return error;
	}
...
String ResourceLoaderCompat::recognize(FileAccess *p_f) {
	error = OK;
