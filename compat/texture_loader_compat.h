...
class ResourceFormatLoaderCompatImage : public CompatFormatLoader {
	GDCLASS(ResourceFormatLoaderCompatImage, CompatFormatLoader);
...
// This is here because Godot 2.x could load pngs, webps, and jpgs directly as ImageTextures, and resources that depend on these will fail to load.
// There are hard-coded checks for the version major and this is disabled on any projects that are > 2.x.
class ResourceFormatLoaderImageTextureCompat : public CompatFormatLoader {
	GDCLASS(ResourceFormatLoaderImageTextureCompat, CompatFormatLoader);
